...
void new_loss_event(struct group_list_t *group, uint16_t txseq)
{
    ...
    if (group->slowstart) {
        ...
        if ((group->seq_wrap > 1) || ((group->seq_wrap == 1) && 
                                  (group->max_txseq >= group->start_txseq))) {
            start_count = count + 1;
        } else {
            start_count = group->start_txseq;
        }
        glog4(group, "start seq for avgbytes:%u\n", start_count);
        bytes = 0;
        ...
        avgbytes = bytes / (group->max_txseq - count);
