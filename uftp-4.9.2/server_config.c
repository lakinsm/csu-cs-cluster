 ...
 *
 *  Copyright (C) 2001-2016   Dennis A. Bush, Jr.   bush@tcnj.edu
 *
...
int rcvbuf, packet_wait, txweight, max_nak_pct, max_passes;
...
void process_args(int argc, char *argv[])
{
    ...
    const char opts[] = "x:R:L:B:g:n:m:Y:h:w:e:ck:K:lTb:t:Q:"
                        "zZI:p:u:j:qfyU:a:H:F:X:M:P:C:D:oE:S:r:s:i:W:N:";
    ...
    while ((c = getopt(argc, argv, opts)) != EOF) {
        ...
        case 'a':
            tmpval = atoi(optarg);
            if ((tmpval <= 0) || (tmpval > 65535)) {
                fprintf(stderr, "Invalid max passes\n");
                exit(ERR_PARAM);
            }
            max_passes = (char)tmpval;
            break;
        case 'U':
            errno = 0;
