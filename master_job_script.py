...
def load_initial_jobs(jobdefs):
    ...
    with open(jobdefs, 'r') as j:
        return deque((x for x in j.read().split() if x and not \
            is_complete('/home/lakinsm/hmm_testing/cs_cluster_files/output/ncba/{}'.format(x.replace('.fasta', '.tblout.scan')))))
...
if __name__ == '__main__':
    ...
    while jobs or any(x for x in worker_status.values()):
        for k, v in worker_status.items():
            ...
            if w_status and v:
                #print(w_status, v[1])
                if not is_complete('/home/lakinsm/hmm_testing/cs_cluster_files/output/ncba/{}'.format(v[0].replace('.fasta', '.tblout.scan'))):
